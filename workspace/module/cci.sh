...
if [ "$#" -lt 3 ] || [ "$#" -gt 4 ]; then
    echo "Usage: $0 outputdir species"
    exit 1
fi
...
# Call python script to generate cci
echo '######################## Generating cci ########################'
/data2/platform/cell_type_workspace/venv/bin/python3 \
    /home/platform/project/cell_type_workspace/cell_type_workspace_api/workspace/module/cci.py \
    --adata_path "$outputdir/adata.h5ad" \
    --output_path "$outputdir" \
    --species "$species"
if [ $? -ne 0 ]; then
    echo "Failed to run the cci.py"
    exit 1
fi
